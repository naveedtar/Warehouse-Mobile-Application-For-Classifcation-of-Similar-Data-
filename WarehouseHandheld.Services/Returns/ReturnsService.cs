...
namespace WarehouseHandheld.Services.Returns
{
    public class ReturnsService : IReturnsService
    {
        ...
        public ReturnsService(WarehouseHandheldService client)
        {
            if (client == null)
        ...
        public async Task<GoodsReturnResponse> PostGoodsReturn(GoodsReturnRequestSync request)
        {
            try
            {
                _conflictStatus = false;
                var _baseUrl = this.Client.BaseUri.AbsoluteUri;
                var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/", StringComparison.Ordinal) ? "" : "/")), WebServiceConfig.PostGoodsReturn).ToString();
                ...
                if (_httpResponse.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    return JsonConvert.DeserializeObject<GoodsReturnResponse>(responseContent);
                }
